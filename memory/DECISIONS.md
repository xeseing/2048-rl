...
### ADR-024 — Stage 2 tuples and the checkpoint format (2026-09-17)
- **Context:** TASK-13 needs 4 × 6-tuples and a run of about 1M games that can survive a crash (SPECS §4.4: checkpoint every 10k games, resumable).
- **Decision:** `STAGE2` grows each Stage 1 shape by one cell: `(0..5)`, `(4..9)`, `(0,1,2,4,5,6)`, `(4,5,6,8,9,10)`. `--stage {1,2}` picks the set. A checkpoint is a single `checkpoint_latest.npz`: the weights (not compressed), plus JSON of the RNG state, the in-flight game slots, the started/done counters, the open metrics window and the elapsed seconds. It is written to a `.tmp` file and then renamed over the old one; only the latest is kept, and it is deleted when the run completes. `--resume` takes every setting from the run's `config.json`, drops metrics rows past the checkpoint's game count, and appends. A resumed run is bit-identical to one that never stopped.
- **Rejected:** Dropping the in-flight games at checkpoint time. It is simpler, but the resumed run would no longer replay the uninterrupted one, and "bit-identical" is the property that makes the resume test meaningful.
